...
def index(request):
    ...
    return render(request, 'include/base.html', {'inc_data': json.dumps(inc_data),
                                                 'inc_col': json.dumps(inc_col),
                                                 'bingtu': json.dumps(bingtu),
                                                 'data': json.dumps(data),
                                                 'col': json.dumps(col),
                                                 'taskdata': json.dumps(taskdata),
                                                 'taskcol': json.dumps(taskcol)})
...
def log_query(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            hosttag = request.POST.get('hosttag', None)
            optype = request.POST.get('optype', None)
            data = func.get_log_data(hosttag, optype, begintime, endtime)

            return render(request, 'log_query.html', {'form': form,
                                                      'objlist': obj_list,
                                                      'optypelist': optype_list,
                                                      'datalist': data,
                                                      'choosed_host': hosttag})
        else:
            print "not valid"
            return render(request, 'log_query.html', {'form': form,
                                                      'objlist': obj_list,
...
def mysql_query(request):
    ...
    if request.method == 'POST':
        ...
        choosed_host = request.POST.get('cx', None)
        ...
        if request.POST.has_key('searchdb'):
            db_se = request.POST.get('searchdbname', None)
            if db_se == 'None':
                db_se = ''
...
def inception(request):
    ...
    if request.method == 'POST':
        ...
        specification = specification[0:30]
        if not User.objects.get(username=request.user.username).db_name_set.filter(dbtag=choosed_host)[:1]:
            form = AddForm()
            upform = Uploadform()
        ...
        if request.POST.has_key('check'):
            ...
            needbackup = (int(request.POST.get('ifbackup', 10)) if int(request.POST.get('ifbackup', 10)) in (0,1) else 1)
...
def get_rollback(request):
    idnum = request.GET.get('taskid', None)
...
def task_manager(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST.has_key('commit'):
            return render(request,'task_manager.html', {'form':form,
                                                        'form2': form2,
                                                        'objlist': obj_list,
                                                        'datalist': data,
                                                        'choosed_host': hosttag})
        elif request.POST.has_key('delete') and (request.user.has_perm('myapp.can_admin_task')
                                                 or request.user.has_perm('myapp.can_delete_task')):
            id_flag = request.POST.get('delete', None)

            if id_flag == 'None':
                pass
            else:
                incept.delete_task(int(id_flag))
            return render(request,'task_manager.html', {'form': form,
                                                       'form2': form2,
                                                       'objlist': obj_list,
                                                       'datalist': data,
            ...
            #id = int(request.POST.get('check', None))
            id_flag = request.POST.get('check', None)
            if id_flag == 'None':
                id = None
            else:
                id = int(id_flag)
            ...
            id_flag = request.POST.get('see_running', None)
            if id_flag == 'None':
                id = None
            else:
                id = int(id_flag)
            ...
            # id = int(request.POST.get('exec', None))
            id_flag = request.POST.get('exec', None)
            if id_flag == 'None':
                id = None
            else:
                id = int(id_flag)
            ...
            sqlsha = request.POST.get('stop', None)
            ...
            # id = int(request.POST.get('appoint', None))
            id_flag = request.POST.get('appoint', None)
            if id_flag == 'None':
                id = None
            else:
                id = int(id_flag)
            ...
            # id = int(request.POST.get('update', None))
            id_flag = request.POST.get('update', None)
            if id_flag == 'None':
                id = None
            else:
                id = int(id_flag)
        ...
        if request.method == 'POST':
            if request.POST.has_key('queryuser'):
                try:
                    ...
                    return render(request, 'previliges/prequery.html', {'inslist': inslist,
                                                                        'pri': pri,
                                                                        'profile': profile,
                                                                        'dbgp': dbgp,
                                                                        'usergp': usergp,
    ...
    if request.method == 'POST':
        ...
        if request.POST.has_key('set'):
            try:
                ...
                return render(request, 'previliges/pre_set.html', {'mail': mail,
                                                                   'username': username,
                                                                   'info': info,
                                                                   'dblist': dblist,
                                                                   'userlist': userlist,
                ...
                return render(request, 'previliges/pre_set.html', {'info': info,
                                                                   'dblist': dblist,
                                                                   'userlist': userlist,
                                                                   'grouplist': grouplist,
                                                                   'usergroup':usergroup})
            ...
            try:
                ...
                return render(request, 'previliges/pre_set.html',{'dblist': dblist,
                                                                  'userlist': userlist,
                                                                  'grouplist': grouplist,
                                                                  'usergroup': usergroup})
            ...
            try:
                ...
                return render(request, 'previliges/pre_set.html', {'info': info,
                                                                   'dblist': dblist,
                                                                   'userlist': userlist,
                                                                   'grouplist': grouplist,'usergroup': usergroup})
            ...
            try:
                ...
                userlist = User.objects.exclude(username=public_user).order_by('username')
                return render(request, 'previliges/pre_set.html', {'user': user,
                                                                   'info': info,
                                                                   'dblist': dblist,
                                                                   'userlist': userlist,
                                                                   'grouplist': grouplist,
    ...
    if request.method == 'POST':
        if request.POST.has_key('query'):
            ...
            try:
                info = "SET OK!"
                insname = Db_instance.objects.get(id=int(request.POST['ins_set']))
                insname = pri.set_ins(insname, request.POST['newinsip'],
                                      request.POST['newinsport'],
                                      request.POST['role'],
                                      request.POST['dbtype'])
                return render(request, 'previliges/set_dbname.html', locals())
            except Exception as e:
            ...
            try:
                info = "CREATE OK!"
                insname = pri.create_dbinstance(request.POST['newinsip'],
                                                request.POST['newinsport'],
                                                request.POST['role'],
                                                request.POST['dbtype'])
                return render(request, 'previliges/set_dbname.html', locals())
            ...
            try:
                ...
                # dbtagname = request.POST['dbtag_set']
                account_set = pri.create_acc(request.POST['newacctag'],
                                             request.POST['newaccuser'],
                                             request.POST['newaccpawd'],
                                             request.POST.getlist('accdb_set'),
                                             request.POST.getlist('accuser_set'),
                                             request.POST['acc_role'])
                return render(request, 'previliges/set_dbname.html', locals())
            except Exception as e:
            ...
            try:
                ...
                account_set = pri.set_acc(old_account,
                                          request.POST['newacctag'],
                                          request.POST['newaccuser'],
                                          request.POST['newaccpawd'],
                                          request.POST.getlist('accdb_set'),
                                          request.POST.getlist('accuser_set'),
...
def fast_dbset(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # print newname_all
            # print "what the fuck"
            info = pri.createdb_fast(ins_set, newinsip, newinsport, newdbtag, newdbname, newname_all, newpass_all,
                                     newname_admin, newpass_admin, newdbtype)
    ...
    if request.method == 'POST':
        try:
            ...
            if request.POST.has_key('show_binary'):
                ...
                parse_binlog.delay(insname, binname, begintime, tbname, dbselected,
                                   request.user.username, countnum, False)
                ...
                tbname = request.POST['tbname']
                dbselected = request.POST['dblist']
                parse_binlog.delay(insname, binname, begintime, tbname,
                                   dbselected, request.user.username, countnum,True)
