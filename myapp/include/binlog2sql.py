...
class Binlog2sql(object):
    ...
    def __init__(self, connectionSettings, startFile=None, startPos=None, endFile=None,
                 endPos=None, startTime=None, stopTime=None, only_schemas=None,
                 only_tables=None, nopk=False, flashback=False, stopnever=False,countnum=10):
        '''
        ...
        self.startTime = datetime.datetime.strptime(startTime, "%Y-%m-%d %H:%M:%S") \
            if startTime else datetime.datetime.strptime('1970-01-01 00:00:00', "%Y-%m-%d %H:%M:%S")

        self.stopTime = datetime.datetime.strptime(stopTime, "%Y-%m-%d %H:%M:%S") \
            if stopTime else datetime.datetime.strptime('2999-12-31 00:00:00', "%Y-%m-%d %H:%M:%S")
        ...
        self.sqllist = []
        self.binlogList = []
        self.connection = mysqldb.connect(**self.connectionSettings)
        try:
            ...
            if not self.serverId:
                raise ValueError('need set server_id in mysql server %s:%s' % (self.connectionSettings['host'],
    ...
    def process_binlog(self):
        ...
        cur = self.connection.cursor()
        tmpFile = create_unique_file('%s.%s' % (self.connectionSettings['host'],
                                                self.connectionSettings['port']))
        ...
        try:
            ...
            for binlogevent in stream:
                ...
                if not self.stopnever:
                    if (stream.log_file == self.endFile and stream.log_pos == self.endPos) \
                ...
                if isinstance(binlogevent, QueryEvent):
                    sql = concat_sql_from_binlogevent(cursor=cur, binlogevent=binlogevent,
                                                      flashback=self.flashback, nopk=self.nopk)
                    if sql:
                        count = count + 1
                        ...
                        or isinstance(binlogevent, UpdateRowsEvent) \
                        or isinstance(binlogevent, DeleteRowsEvent):
...
def command_line_args(args):
    ...
    if (args.startTime and not is_valid_datetime(args.startTime)) \
            or (args.stopTime and not is_valid_datetime(args.stopTime)):
...
def concat_sql_from_binlogevent(cursor, binlogevent, row=None, eStartPos=None, flashback=False, nopk=False):
    ...
    if not (isinstance(binlogevent, WriteRowsEvent)
            or isinstance(binlogevent, UpdateRowsEvent)
            or isinstance(binlogevent, DeleteRowsEvent)
            or isinstance(binlogevent, QueryEvent)):
    ...
    if isinstance(binlogevent, WriteRowsEvent) \
        ...
        sql += ' #start %s end %s time %s' % (eStartPos, binlogevent.packet.log_pos,
                                              datetime.datetime.fromtimestamp(binlogevent.timestamp))
    elif flashback is False \
            and isinstance(binlogevent, QueryEvent) \
            and binlogevent.query != 'BEGIN' \
            and binlogevent.query != 'COMMIT':
